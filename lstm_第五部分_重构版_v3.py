...
def select_typical_points(pred_detail: pd.DataFrame, future_time_cols: List[str], config: VisualConfig) -> pd.DataFrame:
    ...
    # 计算平均未来沉降
    true_cols = [f"true_{c}" for c in future_time_cols if f"true_{c}" in df.columns]
    if not true_cols:
        print("⚠️ pred_detail 没有 true_* 列，无法选择典型点")
        return pd.DataFrame()
    df["mean_true_future"] = df[true_cols].mean(axis=1)

    # 分类选择典型点，每类最多 max_points_per_type 个
    ...
    # 如果没有 point_type 列，创建
    if "point_type" not in chosen.columns:
        chosen["point_type"] = "uplift"
        chosen.loc[chosen["mean_true_future"] <= config.subsidence_threshold, "point_type"] = "subsidence"
        chosen.loc[(chosen["mean_true_future"] > config.subsidence_threshold) & (chosen["mean_true_future"] < config.uplift_threshold), "point_type"] = "transition"

    # 保证 ID/Lon/Lat 列存在
    for col in ["ID", "Lon", "Lat"]:
        if col not in chosen.columns:
            chosen[col] = np.arange(len(chosen))  # 简单填充索引
