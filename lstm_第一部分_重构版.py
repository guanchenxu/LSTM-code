...
def load_and_validate_data(config: Config) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray, List[str], List[str]]:
    ...
    if na_mask.any():
        ...
        if config.allow_drop_na_rows and na_ratio <= 1.0:
            geo_df = geo_df.loc[~na_mask].reset_index(drop=True)
            settle_raw = settle_raw[~na_mask]
            gw_raw = gw_raw[~na_mask]
            print(f"✅ 已删除缺失样本，剩余 {len(geo_df)} 个测点")
        else:
            raise ValueError("数据中存在缺失值，请先清理或允许删除缺失样本")
    ...
    return geo_df, df.loc[geo_df.index].copy(), settle_raw, gw_raw, settle_cols, gw_cols


# ===================== 4. 原始样本构造 =====================
def _history_end_index(config: Config) -> int:
    """
    统一定义“历史窗口最后一个时间步”的全局位置 t_end。

    为了严格支持滞后特征，不再简单使用 0~47 作为所有方案的历史窗口；
    而是让 history_end 依赖 max_lag：
        历史真实沉降时间为 [max_lag, max_lag + L - 1]
    对应 direct 标签为后续 H 个月：
        [max_lag + L, ..., max_lag + L + H - 1]
    ...
    - 整个样本严格无未来信息泄漏
    """
    return config.max_lag + config.history_len - 1


def validate_window_feasibility(config: Config, total_seq_len: int) -> None:
    end_hist = _history_end_index(config)
    if config.pred_mode == "direct":
        last_needed = end_hist + config.horizon
    else:  # rolling 训练标签只需要下一步
        last_needed = end_hist + 1

    if last_needed > total_seq_len - 1:
        raise ValueError(
            f"当前配置无法构造样本：需要访问到索引 {last_needed}，但总长度仅到 {total_seq_len - 1}。"
            f"请减小 history_len / horizon / max_lag。"
        )
...
def build_raw_samples(
    ...
    end_hist = _history_end_index(config)
    hist_times = list(range(end_hist - config.history_len + 1, end_hist + 1))
    ...
    X_raw = np.zeros((num_samples, config.history_len, config.input_dim), dtype=np.float32)
