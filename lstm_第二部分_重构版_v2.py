...
class ModelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        # ---------- 第一阶段核心参数 ----------
        self.history_len_base = int(stage1_cfg.get("history_len", stage1_cfg.get("effective_history_len")))
        self.effective_history_len = int(stage1_cfg.get("effective_history_len", self.history_len_base))
        ...
        # 兼容旧代码字段名
        # L 必须取第一阶段实际建模使用的有效历史长度，而不是基础长度
        self.L = self.effective_history_len
...
class BaseSettleLSTM(nn.Module):
    ...
    说明：
    1. direct 模式下，output_dim = 12（未来12个月）
...
def validate_model_structure(config: ModelConfig, model: nn.Module) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "history_len_base": getattr(config, "history_len_base", config.L),
        "effective_history_len": config.L,
    ...
    if getattr(config, "history_len_base", config.L) != config.L:
        print(f"   基础历史长度：{getattr(config, 'history_len_base', config.L)}")
